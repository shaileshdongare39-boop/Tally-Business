...
# Page Configuration & High-Contrast Professional Theme
st.set_page_config(
...
# Custom High-Contrast CSS for Crisp Visibility & Clean Modern Look
st.markdown("""
    <style>
    /* Main Background & Fonts */
    .stApp { background-color: #f8fafc; color: #0f172a; }
    
    /* Sidebar Styling - High Contrast & High Visibility */
    [data-testid="stSidebar"] {
        background-color: #ffffff !important;
        border-right: 1px solid #e2e8f0;
    }
    
    /* Sidebar Text Fix for Clear Readability */
...
if not st.session_state.user_mobile:
    st.markdown("""
        <div class="main-header">
            ...
            <p>Enterprise Business ERP & Cloud Accounting Solution</p>
        </div>
    """, unsafe_allow_html=True)
        ...
        if not st.session_state.otp_sent:
            if st.button("Send Verification OTP"):
                if len(mobile) == 10 and mobile.isdigit():
                    ...
                    st.info(f"🔑 Testing Verification OTP: **{st.session_state.generated_otp}**")
                else:
                    st.error("Please enter a valid 10-digit mobile number.")
        else:
            otp_in = st.text_input("🔑 Enter 4-Digit OTP")
            role_sel = st.selectbox("Select Role Access", ["Owner", "Salesman / Staff"])
...
# SUBSCRIPTION CHECK & SIDEBAR
sub_status = check_subscription(st.session_state.user_mobile)

st.sidebar.markdown(f"""
    <div class="user-card">
        👤 <b>Account:</b> {st.session_state.user_mobile}
    </div>
    <div class="plan-card">
        🎁 <b>Plan:</b> {sub_status}
    </div>
    <div style="padding: 5px 0px 15px 0px;">
        <h2 style="color: #0f172a; margin: 0; font-size: 1.3rem;">🏢 SD TALLY BUSINESS</h2>
    </div>
""", unsafe_allow_html=True)
...
# EXACT RECOMMENDED MENU ORDER (तुमचा पूर्ण दिलेल्या क्रमानुसार)
...
st.markdown("""
    <div class="main-header">
        <h1>SD TALLY BUSINESS</h1>
        <p>Enterprise Resource Planning & Complete Accounting Suite</p>
    </div>
""", unsafe_allow_html=True)
...
# 2. MASTERS
    ...
    tab1, tab2 = st.tabs(["📦 Item & Stock Master (with Barcode & HSN)", "👤 Party & Ledger Master"])
    conn = get_db()
    c = conn.cursor()
    ...
    with tab1:
        ...
        if st.button("Save Stock Item Master"):
            if i_name:
                try:
                    c.execute("""INSERT INTO inventory (item_name, barcode, hsn_sac, godown, batch_no, expiry_date, sale_price, purchase_price, gst_rate, stock_qty, min_stock_alert)
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                              (i_name, b_code, hsn, godown, batch, str(datetime.now().date()), s_price, p_price, gst, op_stock, 5.0))
                    conn.commit()
            ...
            if p_name:
                try:
                    c.execute("INSERT INTO parties (party_name, gstin, mobile, party_type, opening_balance) VALUES (?, ?, ?, ?, ?)",
                              (p_name, p_gstin, p_mobile, p_type, 0.0))
                    conn.commit()
...
# 4. PURCHASE & GSTR-2B IMPORT
    ...
    st.subheader("📥 GSTR-2B Reconciliation Import")
    st.info("Upload GSTR-2B Excel or CSV file to match Input Tax Credit (ITC):")
    gstr2b_file = st.file_uploader("Upload GSTR-2B File", type=["csv", "xlsx"])
    if gstr2b_file:
        try:
            gstr_df = pd.read_csv(gstr2b_file) if gstr2b_file.name.endswith('.csv') else pd.read_excel(gstr2b_file)
            st.dataframe(gstr_df, use_container_width=True)
            st.success("✅ GSTR-2B Reconciled with Purchases!")
...
# 11. RECEIVABLES & REMINDERS
    ...
    st.subheader("👥 Outstanding Receivables & Reminders")
    conn = get_db()
...
# 17. BRANDING & SIGNATURE
    ...
    st.file_uploader("Upload Digital Signature", type=["png", "jpg"])
